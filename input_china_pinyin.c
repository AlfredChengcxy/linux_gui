                     ...
                     int iFieldNumber, char **ppFieldValue, char **ppFieldName)
{
    ...
    {
        printf("FIELD:%s \t>>>> \tVALUE:%s", ppFieldName[i], ppFieldValue[i]);
        if (i == 0)
        {
            printf("(");
                ...
                int iFieldNumber, char **ppFieldValue, char **ppFieldName)
{
    ...
    if (NULL == pList)
    {
        printf("WordList obj invaild! ['Obj is null']\n");
        return 1;
    }

    if (strlen(ppFieldValue[0]) > UTF8_VALID_MAXLEN)
    {
        printf("Word code invalid! ['Word > 3']\n");
        return 2;
    }
...
int CheckPinyinFile(char *pFileName)
{
    ...
    if (NULL == pFile)
    {
        printf("Import file open failed!\n");
        return 1;
    }
    ...
    while (!feof(pFile))
    {
        ...
        while (*pSrc)   //Extract word & pinyin
        {
            ...
            default:
                if (!iWord)
                {
                    ...
                    {
                        fclose(pFile);
                        printf("File invalid! ['Word > 3' or 'Lost \t']\n");
                        return 2;
                    }
                }
                if (!iPinyin)
        ...
        if (!iWord || !iPinyin)
        {
            ...
            {
                fclose(pFile);
                printf("File invalid! [Blank line]\n");
                return 4;
            }
        }
    }
